/*describe('Authentification', () => {
  ...
  it('Connexion réussie avec credentials valides', () => {
      ...
      cy.get('input[type="password"]').clear().type('Admin@2025!')
      cy.get('button[type="submit"]').click()

      cy.url().should('include', 'http://localhost:5173/')

      cy.contains('Bonjour', { timeout: 15000 }).should('be.visible')
...
describe('Authentification', () => {
  ...
  it('Bloque le compte après 5 tentatives échouées', () => {
    ...
    // Faire 5 tentatives échouées avec un email différent pour ne pas bloquer l'admin
    const lockoutEmail = 'test-lockout@example.com'
    for (let i = 0; i < 5; i++) {
      cy.get('input[type="email"]').clear().type(lockoutEmail)
      cy.get('input[type="password"]').clear().type('WrongPassword')
      cy.get('button[type="submit"]').click()
      cy.wait(500)
    }
